...
public sealed class DisputeAmountHandoverHandler(IAmountHandoverRepository repository) : IRequestHandler<DisputeAmountHandoverCommand, AmountHandoverDto>
{
    public async Task<AmountHandoverDto> Handle(DisputeAmountHandoverCommand request, CancellationToken cancellationToken)
    {
        ...
        if (handover.Status != HandoverStatus.Pending)
            throw new InvalidOperationException("Only pending handovers can be disputed.");

        handover.Dispute(request.Request.ResolutionNotes);
